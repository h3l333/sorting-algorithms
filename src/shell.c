void ordenamientoShell(int arr[], int n) {
  ...
  for (int espacio = n / 2; espacio > 0; espacio /= 2) {
    ...
    // Realizo un ordenamiento por inserción considerando el espacio actual.
    for (int i = espacio; i < n; i++) {

      // Elemento actual que debe ubicarse en su posición correcta.
      int temp = arr[i];
      int j = i;
      ...
      // Coloco temp en su posición definitiva.
      arr[j] = temp;
    }
  }
}